...
class Events extends React.Component{
	...
	async fetchbindAirtable() {
      ...
      .then( r => {return r});
		...
		this.setState({planets, characters, events, locations, campaignData});
	}
	...
	render(){
		...
		<div className='event-page'>
			...
			<div className='event-table'>
				{this.state.events && this.state.events.length > 0 && this.state.events.map(this.renderItem.bind(this))}
			</div>
		</div>
		);
	}
