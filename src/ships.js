...
class Ships extends React.Component{
	...
	async fetchbindAirtable() {
		...
		const characters = await base('Characters').select().all()
			.then( r => {return r});
		const ships = await base('Ships').select().all()
			.then( r => {return r});
      ...
      .then( r => {return r});
		...
		this.setState({planets, characters, events, locations, campaignData, ships});
	}
	...
	render(){
		...
		<div className='page'>
			...
			<div className='ships-table'>
				{this.state.ships && this.state.ships.length > 0 && this.state.ships.map(this.renderItem.bind(this))}
			</div>
		</div>
		);
	}
